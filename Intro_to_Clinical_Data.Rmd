...
```{r}
write.csv(clinic, "/Users/abwilk/Documents/Fall 2023/QBIO 490/qbio_490_andrewwilk/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```
...
#1
#str(clinic)
# there are 1097 rows and 112 coloumns.

#2 
# $gender is character data type.

#3
patient_5_birth_days_to <- clinic[5, "birth_days_to"]
patient_5_age_negative <- (as.integer(patient_5_birth_days_to) %/% 365)
patient_5_age <- patient_5_age_negative * -1
#patient_5_age
# patient 5 is 51 years old.

#4
patient_1_birth_days_to <- as.integer(clinic[1, "birth_days_to"]) * -1
patient_1_last_contact <- as.integer(clinic[1, "last_contact_days_to"])
patient_1_age_last_contact <- patient_1_birth_days_to - patient_1_last_contact
#patient_1_age_last_contact
# patient 1 was 16444 days old at their last follow up.
...
```{r Exercise 4.1}
lymph_nodes_examined <- clinic[,"lymph_nodes_examined_count"]
age_at_diagnosis <- clinic[,"age_at_diagnosis"]

plot(age_at_diagnosis,lymph_nodes_examined, col = "blue")
title("Age at diagnosis vs. # of lymph nodes examined")
...
```{r Exercise 4.2}
# create plot here
vital_status <- clinic$vital_status
age_at_diagnosis <- clinic$age_at_diagnosis
boxplot(age_at_diagnosis ~ vital_status, col ="red")
...
```{r}
under30_over60_mask <- ifelse(clinic$age_at_diagnosis < 35, T, ifelse(clinic$age_at_diagnosis > 60, T,F))
under30_over60_mask
```
...
cleaned_clinic$age_category <- ifelse(
  cleaned_clinic$age_at_diagnosis <= 35, "Young", ifelse(cleaned_clinic$age_at_diagnosis >= 50, 'Old', 'Middle'))
...
cleaned_clinic$last_contact_days_to <- as.numeric(cleaned_clinic$last_contact_days_to)
cleaned_clinic$death_days_to <- as.numeric(cleaned_clinic$death_days_to)

cleaned_clinic$survival_time <- ifelse(
  !is.na(cleaned_clinic$last_contact_days_to),
...
cleaned_clinic$death_event <- ifelse(
  cleaned_clinic$vital_status == "Dead", T, F
)
```
...
The percentage of patients alive at a certain time.
...
15 years ~= 5500 days. Approximate 15-year survival rate for "Old" breast cancer patients is 30%
...
The p-value is 0.2, which is insignificant. It tells us that age is not a strong predictor of breast cancer survival.
